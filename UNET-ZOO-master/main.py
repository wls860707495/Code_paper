...
def getArgs():
    ...
    parse.add_argument('--dataset', default='esophagus',  # dsb2018_256
...
def test(model,val_dataloaders,save_predict=False):
    logging.info('final test........')
    if save_predict ==True:
        dir = os.path.join(r'D:/Project/UNET-ZOO-master/saved_predict',str(args.arch),str(args.batch_size),str(args.epoch),str(args.dataset))
    ...
    model.load_state_dict(torch.load(r'D:/Project/UNET-ZOO-master/saved_model/'+str(args.arch)+'_'+str(args.batch_size)+'_'+str(args.dataset)+'_'+str(args.epoch)+'.pth', map_location='cpu'))  # 载入训练好的模型
    model.eval()
    #plt.ion() #开启动态模式
    with torch.no_grad():
        i=0   #验证集中第i张图
        ...
        for pic,pic_path in val_dataloaders:
            pic = pic.to(device)
            predict = model(pic)
            if args.deepsupervision:
            ...
            ##### dice_total += dice
            scipy.misc.imsave(dir +'/'+ pic_path[0].split('\\')[-1], predict)
            # fig = plt.figure()
            # #print(pic_path[0])
